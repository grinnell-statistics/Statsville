...
public class GameOver : MonoBehaviour {
	...
	public void gmOver(string state){
		if (state != "none") {
            ...
            GameObject.Find("NextDay").GetComponent<Button>().onClick.Invoke();

            if (state.Equals("lost"))
            {
                txt.text = "You Lost...";
                GameObject.Find("WorldModel").GetComponent<WorldModel>().data.AddWinLose("Lose");
            }
            if (state.Equals("won"))
            {
                txt.text = "You Won!";
                GameObject.Find("WorldModel").GetComponent<WorldModel>().data.AddWinLose("Win");
            }

            GameObject.Find("SubmitButton").GetComponent<Button>().onClick.Invoke();
